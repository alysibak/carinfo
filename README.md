...
| Market value | Calibrated against Canadian MSRPs and listing averages (`valuation-calibration.test.ts`, 46 references across mainstream, luxury, performance, pickup, three-row SUV, plug-in hybrid and electric vehicles, all within 25%). Size class is a coarse price signal; trims are distinguished only where the engine gives them away (Mustang GT, Camaro ZL1, Challenger Hellcat, Civic Type R); exotic values are marque-level guesses labelled low confidence. Collector cars (`utils/collector-cars.ts`: first-gen NSX, MkIV Supra, air-cooled 911, Viper, Ford GT, 20-year-old Ferraris and Lamborghinis, hypercars…) are deliberately not valued. |

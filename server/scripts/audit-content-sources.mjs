...
function auditNhtsaCache() {
  ...
  console.log(
    `  with real overall star (>0): ${withRealOverall} (${((withRealOverall / keys.length) * 100).toFixed(1)}%)`,
  );
...
function isPresent(field, v) {
  ...
  const numericZeroMeansEmpty =
    field !== 'guzzler' && field !== 'startStop' && field !== 'fuelType2' && field !== 'atvType';
...
function auditEpaFields() {
  ...
  console.log(
    `Total CSV rows: ${rows.length}; eligible (yr>=1995, not special-purpose): ${eligible.length}`,
  );
  ...
  // EV-specific
  const ev = eligible.filter(
    (r) =>
      (r.atvType || '').toLowerCase() === 'ev' ||
      ((r.fuelType1 || '').toLowerCase().includes('electricity') &&
  ...
  const samples = [
    [
      'Cayenne e-Hybrid 2019 PHEV',
      (r) => r.make === 'Porsche' && r.model === 'Cayenne e-Hybrid' && r.year === '2019',
    ],
    [
      'Prius Prime 2020 PHEV',
      (r) => r.make === 'Toyota' && r.model.includes('Prius Prime') && r.year === '2020',
    ],
    [
      'Camry XSE 2018 gas',
      (r) => r.make === 'Toyota' && r.model === 'Camry XSE' && r.year === '2018',
    ],
    [
      'Model 3 LR 2022 BEV',
      (r) => r.make === 'Tesla' && r.model.includes('Model 3 Long Range') && r.year === '2022',
    ],
  ];
  const showFields = [
    'make',

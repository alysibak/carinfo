...
const { estimateMarketValue, effectiveFuelType } =
  await import('../dist/utils/vehicle-valuation.js');
const { computeOwnershipEconomics, calculateResaleImpact } =
  await import('../dist/utils/ownership-economics.js');
...
for (const [label, find] of SAMPLE) {
  ...
  console.log(
    `  fuel: ${raw.engine.fuelType} → ${normalized.engine.fuelType} (effective: ${effectiveFuelType(normalized)})`,
  );
  console.log(
    `  value: $${Math.round(before.mid / 1000)}k → $${Math.round(after.mid / 1000)}k (${before.low}-${before.high} → ${after.low}-${after.high})`,
  );
  console.log(`  battery: ${after.batteryHealth?.label ?? 'none'}`);
  console.log(
    `  resale 5yr: $${resale.projectedResale5Year.low}-$${resale.projectedResale5Year.high}`,
  );
...
for (const c of cars) {
  ...
  if (
    r.projectedResale5Year.high - r.projectedResale5Year.low < 100 &&
    r.projectedResale5Year.mid < 5000
  )
    degenerate++;
}

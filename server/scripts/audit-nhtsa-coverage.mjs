...
const cache = JSON.parse(
  fs.readFileSync(path.join(root, 'data/raw/nhtsa-enrichment-cache.json'), 'utf8'),
);
...
console.log(
  `Records that match a cached REAL rating: ${recordsWithRating} (${((recordsWithRating / cars.length) * 100).toFixed(1)}%)`,
);
const years = Object.keys(yearHist)
  .map(Number)
  .sort((a, b) => a - b);

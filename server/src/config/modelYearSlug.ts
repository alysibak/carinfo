...
export type SlugWarning = 'drivetrainTokenNotTrailing' | 'unclassifiedParenthetical' | 'emptyBase';
...
const MODEL_SLUG_OVERRIDES: ReadonlyMap<string, string> = new Map([
  ...
  [
    ...
    'charger-2-dr-daytona-scat-pack-track-pack',
  ],
...
export function buildModelYearSlug(
  ...
  const modelSlug = policy === 'identity' && drivetrain !== '' ? `${base}-${drivetrain}` : base;
...
function classifyCollision(distinctRawModels: readonly string[]): CollisionCause {
  const withoutSpecs = new Set(distinctRawModels.map((model) => slugify(stripSpecTokens(model))));
  if (withoutSpecs.size === 1) return 'specToken';

  const withoutTrim = new Set(
    distinctRawModels.map((model) => slugify(splitTrimSuffix(stripSpecTokens(model)).head)),
  );
  if (withoutTrim.size === 1) return 'trim';

  const withoutDrivetrain = new Set(distinctRawModels.map((model) => splitModel(model).base));
...
export function unmappedDrivetrains<T extends VehicleIdentity>(vehicles: readonly T[]): string[] {
